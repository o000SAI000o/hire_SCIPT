...
-- ============================================================================
-- Based on PostgreSQL schema with all V2 fixes applied
-- Includes: Performance indexes, computed columns, triggers, and views
-- ============================================================================
...
-- Create Database
-- ============================================================================
IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = N'ATS_Platform')
BEGIN
...
CREATE TABLE organizations (
    ...
    subscription_plan NVARCHAR(50) DEFAULT 'free' CHECK (subscription_plan IN ('free', 'starter', 'professional', 'enterprise')),
    subscription_status NVARCHAR(50) DEFAULT 'active' CHECK (subscription_status IN ('active', 'cancelled', 'suspended', 'expired')),
...
CREATE TABLE jobs (
    ...
    salary_currency NVARCHAR(10) DEFAULT 'USD',
...
CREATE TABLE candidates (
    ...
    CONSTRAINT FK_candidates_organization FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
);
GO
...
CREATE TABLE interviews (
    ...
    CONSTRAINT FK_interviews_organization FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE TABLE interview_criteria (
    ...
    CONSTRAINT FK_criteria_organization FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
);
GO
...
CREATE TABLE email_queue (
    ...
    workflow_id UNIQUEIDENTIFIER, -- NEW: Track workflow source
...
CREATE TABLE data_retention_policies (
    ...
    CONSTRAINT FK_retention_organization FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
);
GO
...
-- Users
CREATE INDEX idx_users_organization ON users(organization_id) WHERE is_deleted = 0;
CREATE INDEX idx_users_email ON users(email);
CREATE INDEX idx_users_role ON users(role, organization_id) WHERE is_deleted = 0;
GO

-- Jobs
CREATE INDEX idx_jobs_organization ON jobs(organization_id) WHERE is_deleted = 0;
CREATE INDEX idx_jobs_status ON jobs(status, organization_id) WHERE is_deleted = 0;
CREATE INDEX idx_jobs_posted_date ON jobs(posted_date DESC) WHERE is_deleted = 0;
GO

-- Candidates (with full-text search support)
CREATE INDEX idx_candidates_organization ON candidates(organization_id) WHERE is_deleted = 0;
CREATE INDEX idx_candidates_email ON candidates(email);
CREATE INDEX idx_candidates_name ON candidates(name) WHERE is_deleted = 0;
GO

-- Applications (V2 performance indexes)
CREATE INDEX idx_applications_organization ON applications(organization_id) WHERE is_deleted = 0;
...
CREATE INDEX idx_applications_status ON applications(status, organization_id) WHERE is_deleted = 0;
CREATE INDEX idx_applications_job_status_date ON applications(job_id, status, applied_date DESC) WHERE is_deleted = 0;
CREATE INDEX idx_applications_pending_review ON applications(organization_id, status, created_at DESC) WHERE status = 'applied' AND is_deleted = 0;
CREATE INDEX idx_applications_days_since ON applications(days_since_applied) WHERE days_since_applied <= 30 AND is_deleted = 0;
GO

-- Assignments
CREATE INDEX idx_assignments_organization ON assignments(organization_id) WHERE is_deleted = 0;
...
CREATE INDEX idx_interviews_organization ON interviews(organization_id) WHERE is_deleted = 0;
...
CREATE INDEX idx_interviews_status ON interviews(status, organization_id) WHERE is_deleted = 0;
...
-- Files
CREATE INDEX idx_files_organization ON files(organization_id) WHERE is_deleted = 0;
...
CREATE TRIGGER trg_organizations_updated_at ON organizations
AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
...
-- ============================================================================
-- SECTION 5: DASHBOARD METRICS TABLE (instead of materialized view)
-- ============================================================================

-- Since SQL Server doesn't have true materialized views like PostgreSQL,
-- we create a regular table that will be refreshed via scheduled job
...
CREATE TABLE dashboard_metrics (
    ...
    CONSTRAINT FK_dashboard_metrics_org FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
);
GO
...
BEGIN
    ...
    FROM organizations o
    LEFT JOIN jobs j ON o.id = j.organization_id AND j.is_deleted = 0 AND j.status = 'active'
    LEFT JOIN applications a ON j.id = a.job_id AND a.is_deleted = 0
...
-- ============================================================================
-- SECTION 6: INITIAL DATA REFRESH
-- ============================================================================
...
-- ============================================================================
-- SECTION 7: FOREIGN KEY FOR JOBS.ASSIGNMENT_ID (added after assignments table)
-- ============================================================================
...
-- ============================================================================
-- SECTION 8: FOREIGN KEY FOR EMAIL_QUEUE.WORKFLOW_ID (V2 fix)
-- ============================================================================
...
PRINT '3. Review and adjust initial permissions';
